...
public class ObjectFieldValueGeFilterOpInvoker implements ObjectFieldValueFilterOpInvoker {
    ...
    private final Log log = LogFactory.getLog(ObjectFieldValueGeFilterOpInvoker.class);
    ...
    public List<Object> invokeFilter(List<Object> dataSet, ObjectFieldValueFilterOpExpression expression) {
        ...
        log.info("开始执行对象字段值大于等于值过滤");
        if (CollectionUtils.isEmpty(dataSet)) {
            log.info("传入数据集为空");
            return Collections.emptyList();
        }
        AssertUtil.assertTrue(expression.getOpParticipateValue() instanceof Comparable, "运算知类型错误");
        Comparable opParticipateValue = (Comparable) expression.getOpParticipateValue();
        Class<?> pojoClass = dataSet.get(0).getClass();
        return dataSet.stream()
                .filter(data -> opParticipateValue.compareTo(ReflectUtil.reflectInvokePojoGetterMethod(pojoClass, data, expression.getObjectFieldName())) <= 0)
                .collect(Collectors.toList());
    }

    @Override
    public ObjectFieldValueFilterOpType supportObjectFieldValueFilterOpType() {
        return ObjectFieldValueFilterOpType.GE;
    }
}
