...
public class ObjectFieldValueNeFilterOpInvoker implements ObjectFieldValueFilterOpInvoker {
    private final Log log = LogFactory.getLog(ObjectFieldValueNeFilterOpInvoker.class);
    ...
    public List<Object> invokeFilter(List<Object> dataSet, ObjectFieldValueFilterOpExpression expression) {
        ...
        return dataSet.stream()
                .filter(data -> !Objects.equals(ReflectUtil.reflectInvokePojoGetterMethod(pojoClass, data, expression.getObjectFieldName()), expression.getOpParticipateValue())).collect(Collectors.toList());
    }

    @Override
    public ObjectFieldValueFilterOpType supportObjectFieldValueFilterOpType() {
        return ObjectFieldValueFilterOpType.NE;
    }
}
