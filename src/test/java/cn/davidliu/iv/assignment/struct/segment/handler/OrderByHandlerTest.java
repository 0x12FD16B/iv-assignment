...
import cn.davidliu.iv.assignment.struct.expression.OrderByFiledExpression;
import cn.davidliu.iv.assignment.struct.expression.OrderDirection;
...
import static cn.davidliu.iv.assignment.struct.expression.ExpressionLogicOpType.COMMA_SEP;
...
public class OrderByHandlerTest {
    ...
    public void acquireCurrentQuerySegment() {
        ...
        OrderBy orderBy = new OrderBy(builder.firstExpression(new OrderByFiledExpression("field1", OrderDirection.ASC)).buildResult());
    ...
    public void testOrderByIntFieldAsc() {
        ExpressionNodeBuilder builder = new ExpressionNodeBuilder();
        OrderBy orderBy = new OrderBy(builder.firstExpression(new OrderByFiledExpression("intField", OrderDirection.ASC)).buildResult());
        OrderByHandler orderByHandler = new OrderByHandler(orderBy);
    ...
    public void testOrderByIntFieldDesc() {
        ExpressionNodeBuilder builder = new ExpressionNodeBuilder();
        OrderBy orderBy = new OrderBy(builder.firstExpression(new OrderByFiledExpression("intField", OrderDirection.DESC)).buildResult());
        OrderByHandler orderByHandler = new OrderByHandler(orderBy);
    ...
    public void testOrderByMultiFiled() throws NoSuchAlgorithmException {
        ExpressionNodeBuilder builder = new ExpressionNodeBuilder();
        ExpressionNode expressionNode = builder.firstExpression(new OrderByFiledExpression("intField", OrderDirection.DESC))
                .appendExpression(new OrderByFiledExpression("longField", OrderDirection.ASC), COMMA_SEP)
                .buildResult();
